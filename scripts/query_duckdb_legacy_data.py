...
engine = duckdb.connect("~/dbt.duckdb_legacy_emerge")
...
table = execute(
    "SELECT table_name FROM information_schema.tables WHERE table_schema = 'main_gwas'"
)
table
...
table = execute(
    """
    SELECT * FROM main_gwas.cdm_source
...
table = execute(
    "SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'measurement' AND table_schema = 'main_pgrnseq';"
)
table
